...
public interface WeChatAssistant {
    ...
    @SystemMessage("# Role (角色设定)\n" +
            ...
            "- 旅游出行（酒店、机票、火车票、景点、行程规划）优先用 fliggy_ai_search，可补充 meituanTravelTool 交叉验证。用户有预订意图（「帮我订」「预订」「订一间」等）时：先确认目的地、日期、偏好（缺啥问啥）→ 汇总为一句自然语言调用 fliggy_ai_search → 从结果挑 2-4 个最佳，编号列出：名称 ⭐评分 ¥价格 [预订](jumpUrl) → 用户选编号后复述关键信息请确认 → 确认后输出 [一键预订·¥价格](jumpUrl)。换一批就取下一组，结果为空就换目的地或日期，或用 meituanTravelTool 补充。\n" +
            "- 找附近餐厅/外卖：无地址先索要地址 → gaodeMapTool 转经纬度 → foodOrderTool.searchNearbyRestaurants → 结果附导航链接和电话，告知三种方式：①APP搜店名外卖 ②电话点餐 ③导航到店。\n" +
