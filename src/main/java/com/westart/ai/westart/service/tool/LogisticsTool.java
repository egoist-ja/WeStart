...
public class LogisticsTool {
    ...
    public String queryLogistics(
            ...
            @P("收件人手机尾号后4位，选填，部分快递公司如顺丰需要验证手机尾号才能查询") String phone) {
        try {
            ...
            if (apiKey == null || apiKey.isBlank()) {
                log.error("快递物流查询失败，UAPI_KEY 环境变量未设置");
                return "快递物流查询功能尚未配置，暂时无法使用。";
            }
            ...
            try (Response response = okHttpClient.newCall(request).execute()) {
                ...
                if (!response.isSuccessful()) {
                    ...
                    return "快递物流查询失败，快递单号可能不正确或查询服务异常，请稍后重试。";
                }
            ...
            log.error("快递物流查询网络异常，trackingNumber={}，carrierCode={}，phone={}",
                    trackingNumber, carrierCode, phone, e);
            return "快递物流查询暂时不可用，请稍后重试。";
        }
    }
