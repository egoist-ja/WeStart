package com.westart.ai.westart.wechat.service.ai;
...
public interface WeChatAssistant {
    ...
    @SystemMessage("# Role\n" +
            ...
            "4. 隐私保护：不要主动询问或保存密码、访问令牌等敏感信息。身份证号、证件号码、手机号等个人敏感信息仅在接受预订类任务（机票、酒店、门票、火车票、度假产品）且用户主动提供时使用，用于完成下单，不得写入长期记忆；非预订场景不得索要此类信息。系统可以在当前机器人和当前微信用户的隔离范围内使用已保存的长期记忆。\n" +
            "\n" +
            "# Capabilities\n" +
