...
import com.westart.ai.westart.media.service.VoiceGenerateService;
import com.westart.ai.westart.tool.file.FileFormatTool;
...
public class UserMessageServiceImpl implements UserMessageService {
    ...
    private final ChatHistoryConsumerService chatHistoryConsumerService;
    ...
    public void processMessageBatch(
        ...
        ILinkClient client = sessionRegistry.getRequired(sessionId).client();
        try {
            // from_user_id是稳定的用户标识，sessionId只用于定位当前iLink客户端连接。
            ...
            chatHistoryConsumerService.startConsuming(memoryId);
            ...
            String memoryContext = userProfileService.buildProfileContext(memoryId);
            ...
            boolean imageSent = sendToolImages(client, sessionId, userId, result.toolExecutions());
            if (!StringUtils.isBlank(replyContent)) {
                if (replyWithVoice) {
     ...
     */
    private boolean sendToolImages(ILinkClient client, String sessionId,
        ...
        for (ToolExecution execution : toolExecutions) {
            ...
            List<Image> images = extractImages(execution.resultObject());
            for (int index = 0; index < images.size(); index++) {
                Image image = images.get(index);
                byte[] imageBytes = resolveImageBytes(image);
                String fileName = "tool-image-" + (index + 1) + ".png";
                try {
                    client.sendImage(userId, imageBytes, fileName, null);
                    imageSent = true;
                    log.info(
                            "微信工具图片发送成功，sessionId={}，userId={}，imageSize={}",
                            sessionId,
                            userId,
                            imageBytes.length);
                } catch (IOException | ILinkException exception) {
                    throw new IllegalStateException(
                            "微信工具图片发送失败，sessionId="
                                    + sessionId
                                    + "，userId="
                                    + userId,
                            exception);
    ...
    /**
     * 从工具结果中提取图片，非图片结果返回空列表。
     *
     * @param resultObject 工具原始执行结果
     * @return 工具返回的图片列表
     */
    private List<Image> extractImages(Object resultObject) {
        if (resultObject instanceof Image image) {
            return List.of(image);
        }
        if (!(resultObject instanceof List<?> resultList) || resultList.isEmpty()) {
            return List.of();
        }

        List<Image> images = new ArrayList<>(resultList.size());
    ...
    private byte[] resolveImageBytes(Image image) {
        if (!StringUtils.isBlank(image.base64Data())) {
            try {
                ...
                throw new IllegalStateException("工具返回了无效的Base64图片数据", exception);
            }
        }
        if (image.url() == null) {
            throw new IllegalStateException("工具图片结果不包含URL或Base64数据");
        }
        ...
        try (Response response = okHttpClient.newCall(request).execute()) {
            ...
            if (!response.isSuccessful() || responseBody == null) {
                throw new IllegalStateException(
                        "下载工具图片失败，HTTP状态码=" + response.code());
            }
            byte[] imageBytes = responseBody.bytes();
            if (imageBytes.length == 0) {
                throw new IllegalStateException("下载工具图片失败，图片内容为空");
            }
            return imageBytes;
        } catch (IOException exception) {
            throw new IllegalStateException("下载工具图片失败，url=" + image.url(), exception);
        }
    }
